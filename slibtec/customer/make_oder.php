...
if (isset($_POST['make'])) {
  ...
  if (empty($_POST["order_code"]) || empty($_POST["customer_name"]) || empty($_GET['prod_price'])) {
    ...
    // Retrieve customer_id based on customer_name
    $getCustomerIdQuery = "SELECT customer_id FROM rpos_customers WHERE customer_name = ?";
    $getCustomerIdStmt = $mysqli->prepare($getCustomerIdQuery);
    $getCustomerIdStmt->bind_param('s', $customer_name);
    $getCustomerIdStmt->execute();
    $getCustomerIdResult = $getCustomerIdStmt->get_result();

    if ($getCustomerIdResult->num_rows == 1) {
      $row = $getCustomerIdResult->fetch_assoc();
      ...
      if ($postStmt->execute()) {
        $success = "Order Submitted" && header("refresh:1; url=payment1.php");
      } else {
        $err = "Please Try Again Or Try Later";
      }
    } else {
      $err = "Invalid Customer Name";
    }

    // Close the statement
    $getCustomerIdStmt->close();
  }
}
          ...
          <div class="card shadow">
            ...
            <div class="card-body">
              <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row">
                  <div class="col-md-6">
                    <input type="submit" name="make" value="Make Request" class="btn btn-success" value="">
                  </div>
                </div>
              </form>
            </div>
