...
if (isset($_POST['UpdateProduct'])) {
  ...
  if (empty($_POST["prod_code"]) || empty($_POST["prod_name"]) || empty($_POST['prod_desc']) || empty($_POST['prod_price'])) {
    ...
    //QR ADD
    $prod_qr = $_FILES['prod_qr']['name'];
    move_uploaded_file($_FILES["prod_qr"]["tmp_name"], "assets/img/qr/" . $_FILES["prod_qr"]["name"]);   

    $prod_desc = $_POST['prod_desc'];
    $prod_price = $_POST['prod_price'];

    //Insert Captured information to a database table
    $postQuery = "UPDATE rpos_chemicals SET prod_code =?, prod_name =?, prod_img =?, prod_qr =?, prod_desc =?, prod_price =? WHERE prod_id = ?";
    $postStmt = $mysqli->prepare($postQuery);
    //bind paramaters
    $rc = $postStmt->bind_param('sssssss', $prod_code, $prod_name, $prod_img, $prod_qr, $prod_desc, $prod_price, $update);
    $postStmt->execute();
    //declare a varible which will be passed to alert function
    if ($postStmt) {
      $success = "Product Updated" && header("refresh:1; url=payments.php");
    } else {
...
<body>
  ...
  <div class="main-content">
    ...
    $update = $_GET['update'];
    $ret = "SELECT * FROM  rpos_chemicals WHERE prod_id = '$update' ";
                  ...
                  <div class="form-row">
                    <div class="col-md-6">
                     ...
                     <div class="col-md-6">
                      ...
                      <input type="file" name="prod_qr" class="btn btn-outline-warning form-control" value="<?php echo $prod_qr; ?>">
                    </div>
                    <div class="col-md-6">
