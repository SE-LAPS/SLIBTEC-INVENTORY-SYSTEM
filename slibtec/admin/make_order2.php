...
if (isset($_POST['make'])) {
  ...
  if (empty($_POST["order_code"]) || empty($_POST["customer_name"]) || empty($_GET['prod_price'])) {
    ...
    $postQuery = "INSERT INTO rpos_orders2 (prod_qty, order_id, order_code, customer_id, customer_name, prod_id, prod_name, prod_price) VALUES(?,?,?,?,?,?,?,?)";
    ...
    if ($postStmt) {
      $success = "Order Submitted" && header("refresh:1; url=payment2.php");
    } else {
          ...
          <div class="card shadow">
            ...
            <div class="card-body">
              <form method="POST" enctype="multipart/form-data">
                ...
                <?php
                $prod_id = $_GET['prod_id'];
                $ret = "SELECT * FROM  rpos_chemicals WHERE prod_id = '$prod_id'";
                $stmt = $mysqli->prepare($ret);
                $stmt->execute();
                $res = $stmt->get_result();
                ...
                <div class="form-row">
                  <div class="col-md-6">
                    <input type="submit" name="make" value="Make Product" class="btn btn-success" value="">
